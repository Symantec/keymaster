// This file contents are mostly copies from:
// https://github.com/tstranex/u2f/blob/master/certs.go
//
...
const FtFido0200Cert = `-----BEGIN CERTIFICATE-----
...
func mustLoadPool2(pemCerts []byte) *x509.CertPool {
	p := x509.NewCertPool()
	if !p.AppendCertsFromPEM(pemCerts) {
		log.Fatal("u2f copy: Error loading root cert pool.")
		return nil
	}
	return p
}

var u2froots = mustLoadPool2([]byte(yubicoRootCert + entersektCert + neowaveCert + plugUpCert + FtFido0200Cert))
